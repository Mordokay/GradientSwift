...
open class ConicalGradientLayer: CALayer {
    ...
    // MARK: Properties
    /// the size from the outer ring to the inner ring
    open var ringThickness: CGFloat = 10 {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    private func draw(in rect: CGRect) {
        ...
        let inset: CGFloat = ringThickness// adjust as necessary for more or less meaty donuts
